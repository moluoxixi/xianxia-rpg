...
import type { NPC, Scene } from '@shared/types/scene';
...
export function applyResourceChanges(state: GameState, changes: ResourceChange[]): ApplyResourceResult {
  ...
  for (const change of changes) {
    ...
    if (change.type === 'realm' && typeof change.value === 'string') breakthroughRealm = change.value;
    events.push({
      id: crypto.randomUUID(),
      runId: next.runId,
      type: change.type,
...
function adjudicateResourceChange(state: GameState, change: ResourceChange): AdjudicationResult {
  ...
  if (change.type === 'item_remove') return adjudicateItemRemove(state, change);
  if (change.type === 'location') return adjudicateLocation(state, change);
  if (change.type === 'npc_add') return adjudicateNpcAdd(state, change);
  if (change.type === 'npc_update') return adjudicateNpcUpdate(state, change);
  if (change.type === 'favorability') return adjudicateFavorability(state, change);
  if (change.type === 'combat') return adjudicateCombat(state, change);
  if (change.type === 'item_add') return adjudicateOverlevelChance(state, resourceRealmReq[change.name ?? ''], `越级机缘「${change.name ?? '未知物品'}」`);
  if (change.type === 'skill_add') return adjudicateOverlevelChance(state, skillRealmReq[change.name ?? ''], `越级功法「${change.name ?? '未知功法'}」`);
  if (change.type === 'realm') return adjudicateRealm(state, change);
...
function adjudicateItemRemove(state: GameState, change: ResourceChange): AdjudicationResult {
  ...
  if (!item || item.count < count) return { accepted: false, reason: `背包中没有足够的「${name || '未知物品'}」，无法扣除。` };
  return { accepted: true };
}
...
function adjudicateNpcAdd(state: GameState, change: ResourceChange): AdjudicationResult {
  ...
  if (npcId && state.defeatedNpcs.includes(npcId)) return { accepted: false, reason: `「${change.npc_name ?? npcId}」已被记录为败亡或离场，不能无代价重新出现。` };
  if (npcId && state.npcs[npcId]) return { accepted: false, reason: `NPC「${state.npcs[npcId].name}」已存在，新增请求应改为更新。` };
...
function adjudicateNpcUpdate(state: GameState, change: ResourceChange): AdjudicationResult {
  ...
  if (!npc) return { accepted: false, reason: '尝试更新不存在的 NPC，已忽略。' };
  if (state.defeatedNpcs.includes(npc.id)) return { accepted: false, reason: `「${npc.name}」已败亡或离场，不能继续更新状态。` };
  if (change.npc_location && !state.scenes[change.npc_location]) return { accepted: false, reason: `目标场景「${change.npc_location}」尚未被定义，NPC 无法迁移。` };
  return { accepted: true };
}
...
function adjudicateFavorability(state: GameState, change: ResourceChange): AdjudicationResult {
  ...
  if (!npc) return { accepted: false, reason: '目标 NPC 不存在，无法改变好感度。' };
  if (state.defeatedNpcs.includes(npc.id)) return { accepted: false, reason: `「${npc.name}」已败亡或离场，好感度不再变化。` };
  return { accepted: true };
}
...
function adjudicateRealm(state: GameState, change: ResourceChange): AdjudicationResult {
  ...
  if (targetLevel <= currentLevel + 1) return { accepted: true };
...
function adjudicateOverlevelChance(state: GameState, requiredRealm: string | undefined, label: string): AdjudicationResult {
  if (!requiredRealm) return { accepted: true };
...
function rollChance(probability: number, failReason: string, reportedProbability = probability): AdjudicationResult {
  ...
  if (accepted) return { accepted: true, reason: `低概率机缘成立（约${Math.round(reportedProbability * 100)}%）。`, probability };
