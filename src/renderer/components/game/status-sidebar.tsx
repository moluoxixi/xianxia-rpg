...
import { getItemIcon, itemDescriptions, pinnedItems, statPercent } from '@/domain/game-data';
    ...
    <aside className="w-[320px] shrink-0 overflow-y-auto bg-[#14142e] p-4">
      ...
      <Card className="mb-4 p-3">
        ...
        <div className="flex flex-col gap-1.5">
          {gameState.skills.map((skill) => <div key={skill.id ?? skill.name} className="rounded bg-secondary px-2 py-1.5 text-xs text-foreground/80">{skill.name}（{skill.level}）</div>)}
        </div>
      </Card>
...
function PanelTitle({ icon, title, action }: { icon: ReactNode; title: string; action?: string }) {
  return <div className="mb-2.5 flex items-center justify-between border-b border-border pb-1.5 text-sm font-semibold text-primary"><span className="flex items-center gap-2">{icon}{title}</span>{action ? <span className="text-[10px] font-normal text-muted-foreground">{action}</span> : null}</div>;
}
...
function StatBar({ icon, label, value, max, className }: { icon: ReactNode; label: string; value: number; max: number; className: string }) {
  return <div className="mb-3"><div className="mb-1 flex justify-between text-xs"><span className="flex items-center gap-1 text-muted-foreground">{icon}{label}</span><span>{value}/{max}</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full bg-gradient-to-r transition-all ${className}`} style={{ width: statPercent(value, max) }} /></div></div>;
}
