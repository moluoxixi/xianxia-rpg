...
export function MessageBubble({ message }: { message: ChatMessage }) {
  const meta = message.role === 'user'
    ? { name: '韩立', icon: <User className="h-5 w-5" />, color: 'text-foreground', align: 'justify-end' }
    : message.role === 'system'
      ? { name: '天道', icon: <span className="text-lg">卷</span>, color: 'text-system-gold', align: 'justify-start' }
      : { name: 'AI', icon: <Sparkles className="h-5 w-5" />, color: 'text-ai-blue', align: 'justify-start' };
